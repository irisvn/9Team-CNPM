...
$fields = array(
	...
	'min_amount' => array(
		...
		'default'           => 0,
		'description'       => apply_filters( 'alg_wc_custom_payment_gateways_settings', sprintf(
	...
	'default_order_status' => array(
		...
		'description'       => sprintf( 'In case you need more custom order statuses - we suggest using free <a target="_blank" href="%s">Order Status for WooCommerce plugin</a>.',
			'https://wordpress.org/plugins/order-status-for-woocommerce/' ),
	...
	'send_email_to_admin' => array(
		...
		'description'       => sprintf( __( 'This may help if you are using pending or custom default order status and not receiving %s emails.', 'custom-payment-gateways-woocommerce' ),
	...
	'custom_return_url' => array(
		...
		'description'       => __( 'Optional. Leave blank to use default URL.', 'custom-payment-gateways-woocommerce' ) . ' ' .
			sprintf( __( 'Available placeholders: %s.', 'custom-payment-gateways-woocommerce' ),
				'<code>' . implode( '</code>, <code>', array( '%order_id%', '%order_key%' ) ) . '</code>' ),
		'default'           => '',
		'type'              => 'text',
		'css'               => 'width:100%',
	),
	...
	$fields = array_merge( $fields, array(
		'fees_options' => array(
			...
			'description'       => __( 'This section allows you to set extra checkout fees.', 'custom-payment-gateways-woocommerce' ) . ' ' .
				sprintf( __( 'General options are in %s.', 'custom-payment-gateways-woocommerce' ),
		...
		'fees_total' => array(
			...
			'description'       => apply_filters( 'alg_wc_custom_payment_gateways_settings',
				sprintf( '<div style="' . $div_style  . '">You will need <a target="_blank" href="%s">Custom Payment Gateways for WooCommerce Pro plugin</a> to add more than one fee.</div>',
					'https://wpfactory.com/item/custom-payment-gateways-woocommerce/' ), 'total_number' ),
			'default'           => 1,
			'type'              => 'number',
	...
	for ( $i = 1; $i <= apply_filters( 'alg_wc_custom_payment_gateways_values', 1, 'total_fees', $this ); $i++ ) {
		$fields = array_merge( $fields, array(
			...
			'fee_amount_' . $i => array(
				...
				'type'          => 'number',
				'default'       => '',
				'custom_attributes' => array( 'step' => '0.00001', 'min' => 0 ),
			),
...
if ( 'yes' === get_option( 'alg_wc_cpg_input_fields_enabled', 'yes' ) ) {
	$fields = array_merge( $fields, array(
		...
		'input_fields_total' => array(
			...
			'desc_tip'          => __( 'Save changes to see new options.', 'custom-payment-gateways-woocommerce' ),
			'description'       => apply_filters( 'alg_wc_custom_payment_gateways_settings',
				sprintf( '<div style="' . $div_style  . '">You will need <a target="_blank" href="%s">Custom Payment Gateways for WooCommerce Pro plugin</a> to add more than one input field.</div>',
					'https://wpfactory.com/item/custom-payment-gateways-woocommerce/' ), 'total_number' ),
			'default'           => 1,
			'type'              => 'number',
